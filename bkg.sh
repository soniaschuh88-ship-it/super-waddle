#!/usr/bin/env bash
# =============================================================================
# bkg.sh — bKG Process Manager
#
# Usage:
#   ./bkg.sh start      Start the app (kills any existing instance first)
#   ./bkg.sh stop       Stop everything
#   ./bkg.sh restart    Stop then start
#   ./bkg.sh status     Show running services and URLs
#   ./bkg.sh build      Build the production bundle
#   ./bkg.sh dev        Run Vite dev server (no build needed)
#   ./bkg.sh logs       Tail all log files
#   ./bkg.sh logs serve Tail a specific log  (serve|tunnel|llama|ollama)
#
...
cmd_start() {
  ...
  echo ""
  echo -e "  ${DIM}Stop with: ./bkg.sh stop${NC}"
}
...
${BOLD}bKG — Process Manager${NC}
  ...
  ${CYAN}./bkg.sh start${NC}       Build (if needed) and start app + tunnel
  ${CYAN}./bkg.sh stop${NC}        Stop all bKG processes
  ${CYAN}./bkg.sh restart${NC}     Stop then start
  ${CYAN}./bkg.sh status${NC}      Show running services and public URL
  ${CYAN}./bkg.sh build${NC}       Rebuild production bundle only
  ${CYAN}./bkg.sh dev${NC}         Start Vite dev server (hot-reload)
  ${CYAN}./bkg.sh logs${NC}        Tail all log files
  ${CYAN}./bkg.sh logs serve${NC}  Tail a specific log (serve|tunnel|dev|build)
...
${BOLD}Examples:${NC}
  ./bkg.sh start                          # production
  ICADP_TUNNEL=0 ./bkg.sh start          # no public tunnel
  ICADP_NO_BUILD=1 ./bkg.sh restart      # restart, skip build
  ./bkg.sh dev                            # dev with hot-reload
  ./bkg.sh logs serve                     # tail server logs
