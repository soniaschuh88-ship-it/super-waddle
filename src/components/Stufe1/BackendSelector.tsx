/** src/components/Stufe1/BackendSelector.tsx – Backend picker (WebGPU/MLC/llama.node/Ollama). */
...
import { pingRestBackend, OLLAMA_POPULAR_MODELS } from '@/lib/llm-client';
import type { BackendType, BackendConfig } from '@/types';

interface BackendDef { type: BackendType; label: string; tagline: string; icon: React.FC<{size?:number;className?:string}>; defaultUrl: string; defaultModel: string; }

const BACKENDS: BackendDef[] = [
  { type:'webgpu',      label:'WebGPU',          tagline:'In-browser · no server · Chrome/Edge', icon:Cpu,       defaultUrl:'',                       defaultModel:'Qwen2.5-1.5B-Instruct-q4f16_1-MLC' },
  { type:'ollama',      label:'Ollama',           tagline:'Local · one-click pull · CPU/GPU',     icon:HardDrive, defaultUrl:'http://localhost:11434',  defaultModel:'qwen2.5:1.5b' },
  { type:'mlc-server',  label:'MLC-LLM Server',  tagline:'mlc_llm serve · OpenAI REST',          icon:Server,    defaultUrl:'http://localhost:8000',   defaultModel:'Qwen2.5-1.5B-Instruct-q4f16_1-MLC' },
  { type:'llama-node',  label:'llama.node',       tagline:'server/index.js · GGUF file',          icon:HardDrive, defaultUrl:'http://localhost:8001',   defaultModel:'' },
];
...
export function BackendSelector() {
  ...
  const [ping, setPing]             = useState<PingStatus>('idle');

  const commit = useCallback((partial: Partial<BackendConfig>) => {
    dispatch({ type:'SET_BACKEND', config:{ type:partial.type??backendConfig.type, serverUrl:partial.serverUrl??draftUrl, modelId:partial.modelId??draftModel } });
  ...
  const active = BACKENDS.find(b=>b.type===backendConfig.type)!;
    ...
    <div className="rounded-lg border border-border bg-surface overflow-hidden">
      <button onClick={()=>setExpanded(p=>!p)} className="w-full flex items-center gap-3 px-4 py-2.5 text-left hover:bg-panel transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="ml-2 text-[11px] text-muted">{active.tagline}</span>
        </div>
      ...
      {expanded && (
        <div className="border-t border-border px-4 py-3 flex flex-col gap-3">
          ...
          <div className="flex gap-2 flex-wrap">
            {BACKENDS.map(def => {
              const BIcon = def.icon; const isActive = def.type===backendConfig.type;
              return (
                <button key={def.type} onClick={()=>selectType(def)}
                  className={['flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all',
                    isActive?'bg-accent/15 border-accent/50 text-accent':'bg-base border-border text-muted hover:border-accent/30 hover:text-text-primary'].join(' ')}>
                  <BIcon size={12}/>{def.label}
                </button>
              );
            })}
          </div>
          ...
          {backendConfig.type==='ollama' && (
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-semibold text-muted uppercase tracking-wider">Model (must be pulled)</label>
                ...
                {OLLAMA_POPULAR_MODELS.map(m=>(
                  <button key={m.name} onClick={()=>{setDraftModel(m.name);commit({modelId:m.name});}}
                    className={['px-2 py-1.5 rounded-lg text-[11px] font-mono border transition-colors text-left',
                      draftModel===m.name?'bg-accent/15 border-accent/40 text-accent':'bg-base border-border text-muted hover:border-accent/30 hover:text-text-primary'].join(' ')}>
                    <div className="font-semibold">{m.name}</div>
                    <div className="text-muted/60 text-[10px]">{m.description}</div>
                  </button>
                ))}
              </div>
              <p className="text-[11px] text-muted/60">Pull models in the <a href="/admin" className="text-accent hover:underline">Admin Dashboard</a> if not yet available.</p>
            </div>
          )}
          ...
          {isRest && (
            <div className="grid grid-cols-2 gap-2">
              ...
              {backendConfig.type!=='ollama' && (
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-semibold text-muted uppercase tracking-wider">{backendConfig.type==='llama-node'?'GGUF Path':'Model ID'}</label>
                  <input type="text" value={draftModel} onChange={e=>setDraftModel(e.target.value)} onBlur={()=>commit({modelId:draftModel})}
                    placeholder={backendConfig.type==='llama-node'?'/path/to/model.gguf':active.defaultModel}
                    className="bg-base border border-border text-text-primary text-xs font-mono rounded-lg px-3 py-1.5 focus:outline-none focus:border-accent/60 placeholder:text-muted/40"/>
                </div>
              )}
            </div>
          ...
          {isRest && (
            <div className="flex items-center gap-3">
              ...
              <p className="text-[11px] text-muted/60">
                {backendConfig.type==='ollama'?'Run: ollama serve':backendConfig.type==='llama-node'?'Run: cd server && MODEL_PATH=… node index.js':'Run: mlc_llm serve <model>'}
              </p>
            </div>
          )}
        </div>
